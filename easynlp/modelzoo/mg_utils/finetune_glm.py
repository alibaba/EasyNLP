...
from easynlp.utils.arguments import get_ds_args as get_args
from filelock import FileLock
from easynlp.appzoo.sequence_generation.mg_seq2seq.data_utils import build_data_loader, FakeDataloader
...
def _train(model, optimizer, lr_scheduler, forward_step,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for iteration_, batch in enumerate(train_dataloader[0]):
            ...
            # Evaluation
            if end_of_epoch_callback is not None and args.eval_interval and args.iteration % args.eval_interval == 0:
...
def finetune(args, train_valid_datasets_provider, model_kwargs, forward_step=finetune_forward_step,
    ...
    if end_of_epoch_callback_provider is not None:
        ...
        if args.mode == 'train' and len(args.tables.split(',')) == 3:
    ...
    if torch.distributed.get_rank() == 0:
        ...
        print_and_save_args(args, verbose=True, log_dir=args.checkpoint_dir)
    ...
    if train_dataloader is not None and args.epochs > 0:
        ...
        if end_of_train_callback is not None:
            score_dict = end_of_train_callback(model, epoch=-1, output_predictions=False)
    # Or just evaluate.
    else:
        if end_of_train_callback is not None and args.mode == 'evaluate':
            print_rank_0('evaluation only mode, setting epoch to -1')
            score_dict = end_of_train_callback(model, epoch=-1, output_predictions=False)
        else:
            print_rank_0('prediction only mode, setting epoch to -1')
            score_dict = end_of_train_callback(model, epoch=-1, output_predictions=True)
    # if score_dict is not None and torch.distributed.get_rank() == 0 and args.mode != 'predict':
...
if __name__ == '__main__':
    ...
    # el
    if args.task.lower() in ['cnn_dm', 'cnn_dm_original', 'gigaword', 'blank', 'squad_generation', 'squad',
                               'squad_v1', 'xsum', 'extraction', 'cmrc', 'chinesegen']:
