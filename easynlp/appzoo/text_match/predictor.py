...
class TextMatchPredictor(Predictor):
    ...
    def preprocess(self, in_data):
        ...
        for record in in_data:
            ...
            try:
                self.MUTEX.acquire()
                feature = self.tokenizer(text_a, text_b, padding='max_length', truncation=True,
...
class FewshotSingleTowerTextMatchPredictor(PromptPredictor):
    pass

class CptFewshotSingleTowerTextMatchPredictor(CPTPredictor):
    pass

class TextMatchTwoTowerPredictor(Predictor):
    def __init__(self, model_dir, model_cls=None, *args, **kwargs):
        super(TextMatchTwoTowerPredictor, self).__init__(*args, **kwargs)
    ...
    def preprocess(self, in_data):
        ...
        for record in in_data:
            ...
            try:
                ...
                self.MUTEX.release()
            real_length = len(feature["input_ids"])
            padding_length = max_seq_length - real_length
            feature["input_ids"] += [0] * padding_length
    ...
    def postprocess(self, result):
        emb_output = result["emb_output"]
        new_results = list()
        for b, preds in enumerate(emb_output):
            new_results.append({
                "emb_output":  ",".join([str(t) for t in result["emb_output"][b]])
            })
        if len(new_results) == 1:
            new_results = new_results[0]
        return new_results
