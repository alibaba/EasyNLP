...
class Text2VideoRetrievalEvaluator(Evaluator):
    ...
    def evaluate(self, model):
        ...
        video_embeds_all=[]
        text_embeds_all=[]
        print('Start calculating features of samples ...')
        for _step, batch in tqdm.tqdm(enumerate(self.valid_loader)):
            infer_start_time = time.time()
            with torch.no_grad():
                outputs = model(batch)
            infer_end_time = time.time()
            total_spent_time += infer_end_time - infer_start_time
            video_embeds_all.append(outputs['video_embeds'])
            text_embeds_all.append(outputs['text_embeds'])
        video_embeds_tensor=torch.cat(video_embeds_all,dim=0)
        text_embeds_tensor=torch.cat(text_embeds_all,dim=0)
        query_len=text_embeds_tensor.size()[0]
        agreement=text_embeds_tensor@video_embeds_tensor.t()
        agreement_size=agreement.size()
        r1_stat, r5_stat, r10_stat = 0, 0, 0
        print('Start calculating recall score ...')
        for idx in tqdm.tqdm(range(0,agreement_size[0])):
