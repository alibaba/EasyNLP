...
from data_utils.utils import data_to_name
...
from pet.modeling import train_pet, train_pet_cross
...
def main():
    ...
    parser.add_argument("--task_type", default='single_task', type=str, required=False, choices=['single_task', 'cross_task'],
    ...
    # edit by wjn
    if args.task_type is None or args.task_type == 'single_task':
        # 如果是single task，则只针对此时task_name对应的task进行训练和测试
        ...
        train_pet(eval_data=eval_data, # 相当于测试集
                      dev32_data=dev32_data, # 相当于验证集
                      ...
                      seed=args.seed)
    elif args.task_type == 'cross_task':
        # 如果是cross_task，则先加载一个group内的数据（数据已由LM-BFF模型内的数据生成预先生成好了）
        train_data = load_examples(
            args.task_name, args.data_dir, TRAIN_SET, num_examples=-1, num_examples_per_label=None)
        ...
        train_pet_cross(dev32_data=dev_data,  # 相当于验证集
                  train_data=train_data,  # 相当于训练集
                  train_config=pet_train_cfg,
                  eval_config=pet_eval_cfg,
                  model_config=pet_model_cfg,
