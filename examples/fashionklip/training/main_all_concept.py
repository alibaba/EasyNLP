...
from training.train_all_concept import train, evaluate
from training.train_epoch import train_only
from training.data import get_data, get_eval_img_dataset_list, get_eval_txt_dataset
...
def main_worker(gpu, ngpus_per_node, log_queue, args):
    ...
    if args.openai_pretrained:
        print("Start loading pretrained CLIP...")
    ...
    img_data_sets = get_eval_img_dataset_list(args)
    concept_data = get_eval_txt_dataset(args)
    kb_concept_data = get_eval_txt_dataset(args, is_kb=True)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.is_finetune_only and epoch < 10:
            print((f'Start training epoch {epoch} w/o knowledge...'))
            train_only(model, data, epoch, optimizer, scaler, scheduler, args, writer)
        else:
            print((f'Start training epoch {epoch} with knowledge...'))
            train(model, data, img_data_sets, concept_data, kb_concept_data, embed_dim, epoch, optimizer, scaler, scheduler, args, writer)
