...
def run_tip_adapter_F(cfg, clss_head, test_features, test_labels, clip_weights,
    ...
    optimizer = torch.optim.AdamW(
        ...
        lr=cfg['lr'] , eps=1e-4) # 2e-316 shot72.81
    ...
    for train_idx in range(cfg['train_epoch'] ):
        ...
        for i, (images, target) in enumerate(tqdm(train_loader_F)):
            ...
            img_feat = image_features.unsqueeze(1).repeat(1, 47, 1)
        ...
        # Eval
        adapter.eval()
        test_img_feat = test_features.unsqueeze(1).repeat(1, 47, 1)
...
def main():
    ...
    train_loader_F = build_data_loader(data_source=dataset.train_x, batch_size=cfg['batch_size'], tfm=train_tranform, is_train=True,
                                       shuffle=True)
