...
def run_tip_adapter_F(cfg, clss_head, test_features, test_labels, clip_weights,
    ...
    adapter = Adapter(512, 4).to(clip_model.dtype).cuda(device_id)
    ...
    for train_idx in range(max_epoch):
        # Train
        adapter.train()
        clip_model.eval()
        correct_samples, all_samples = 0, 0
        loss_list = []
        print('Train Epoch: {:} / {:}'.format(train_idx, max_epoch))
        ...
        for i, (images, target) in enumerate(tqdm(train_loader_F)):
            ...
            ratio = 0.2
            adapt_image_features = ratio * x + (1 - ratio) * image_features
            adapt_image_features = adapt_image_features / adapt_image_features.norm(dim=-1, keepdim=True)
            logits = clip_model.logit_scale.exp() * adapt_image_features @ clip_weights
        ...
        adapter.eval()
        clip_model.eval()
        x = adapter(test_features)
        ratio = 0.2
        adapt_test_features = ratio * x + (1 - ratio) * test_features
        adapt_test_features = adapt_test_features / adapt_test_features.norm(dim=-1, keepdim=True)
        test_logits = adapt_test_features @ clip_weights
...
def main():
    ...
    train_loader_F = build_data_loader(data_source=dataset.train_x, batch_size=32, tfm=train_tranform, is_train=True,
                                       shuffle=True)
