...
def run_tip_adapter(clip_model, up_head, down_head, train_loader, test_loader, max_epoch = 200, lr=5e-7):
    ...
    optimizer = torch.optim.AdamW(
        [
            ...
            {"params": up_head.parameters(), "lr": 2e-3},
            {"params": down_head.parameters(), "lr": 2e-3},
        ],
        lr=lr,
        betas=(0.9, 0.999),
        eps=1e-4,
    ...
    for epoch in range(0, max_epoch):
        ...
        for i, (image, text, idx) in enumerate(train_loader):
            ...
            match_loss = matching_loss(image_features, hypo_feature, up_head, down_head, device_id)
            # #### matching loss
...
def matching_loss(image_features, hypo_feature, up_head, down_head, device_id):
    ...
    for b in range(bs):
        ...
        pos_fusion_feat.append(torch.cat([pos_feat, img_feat], dim=-1))
        neg_fusion_feat.append(torch.cat([neg_feat, img_feat.repeat(bs-1, 1)], dim=-1))
    ...
    all_out = up_head(torch.cat([all_pos_feat, all_neg_feat], dim=0).float())
    all_out = F.relu(all_out)
    logits = down_head(all_out)
    prob_logits = F.softmax(logits, dim=-1)
    all_pos_scores = prob_logits.gather(-1, contra_lbl.unsqueeze(-1))
    ones = torch.ones(all_pos_scores.size()).cuda(all_pos_scores.device)
    all_contra = loss_func(all_pos_scores.repeat(1, 2), prob_logits, ones)

    # matching_loss = F.cross_entropy(output, itm_labels) + all_contra * 0.1 #loss_func(pos_scores.repeat(1, 2), output, ones)
    matching_loss = F.cross_entropy(all_out, contra_lbl) + all_contra
...
def main():
    ...
    clip_model.eval()
    embed_dim = 512
    itm_head_up = nn.Linear(embed_dim*2, embed_dim*4, bias=False)
    itm_head_down = nn.Linear(embed_dim*4, 2, bias=False)
    itm_head_up.to(clip_model.positional_embedding.device)
    itm_head_down.to(clip_model.positional_embedding.device)
    ...
    # ------------------------------------------ Tip-Adapter ------------------------------------------
    run_tip_adapter(clip_model, itm_head_up, itm_head_down, train_loader, test_loader)
