...
def pre_load_features(cfg, split, clip_model, loader, load_cache_layer=False):
    ...
    if cfg['load_pre_feat'] == False:
        ...
        img_feature, labels, txt_feature = torch.cat(features), torch.cat(labels), torch.cat(textual_feature)
        ...
        torch.save(img_feature.cpu(), cfg['cache_dir'] + "/img" + split + "_feature.pt")
        torch.save(txt_feature.cpu(), cfg['cache_dir'] + "/txt" + split + "_feature.pt")
        torch.save(labels.cpu(), cfg['cache_dir'] + "/" + split + "_label.pt")
    else:
        img_feature = torch.load(cfg['cache_dir'] + "/img" + split + "_feature.pt")
        txt_feature = torch.load(cfg['cache_dir'] + "/txt" + split + "_feature.pt")
        labels = torch.load(cfg['cache_dir'] + "/" + split + "_label.pt")
    return img_feature.to(device_id), txt_feature.to(device_id), labels.to(device_id)
