...
def run_tip_adapter_F(cfg, cache_keys, cache_values, test_features, test_labels, clip_model, train_loader_F):
    ...
    for train_idx in range(cfg['train_epoch']):
        ...
        for i, (images, questions, target) in enumerate(tqdm(train_loader_F)):
            ...
            pos_scores = cache_logits.gather(-1, target.unsqueeze(-1))
            neg_scores = cache_logits.gather(-1, (1 - target).unsqueeze(-1))
            ones = torch.ones(pos_scores.size()).cuda(cache_logits.device)
            loss_func = torch.nn.MarginRankingLoss(0.0)
            # equal_prob = torch.cosine_similarity(image_features, hypo_feature)
            # loss = F.mse_loss(equal_prob, target.to(torch.float16))
            loss = F.cross_entropy(cache_logits, target) + loss_func(pos_scores, neg_scores, ones)
