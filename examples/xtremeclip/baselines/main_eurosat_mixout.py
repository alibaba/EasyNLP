...
from main_visualentailment_mixout import PriorWD
...
def run_tip_adapter_F(cfg, clss_head, test_features, test_labels, clip_weights,
    ...
    for sup_module in clip_model.modules():
        for name, module in sup_module.named_children():
            if isinstance(module, nn.Dropout):
                module.p = 0.0
            if isinstance(module, nn.Linear):
                target_state_dict = module.state_dict()
                data_type = module.weight.dtype
                bias = True if module.bias is not None else False
                new_module = MixLinear(
                    module.in_features, module.out_features, bias, target_state_dict["weight"], 0.7
                ).to(device_id)
                new_module.load_state_dict(target_state_dict)
                new_module.to(data_type)
