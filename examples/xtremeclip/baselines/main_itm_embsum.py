...
def run_tip_adapter(clip_model, img_head, txt_head, train_loader, test_loader, max_epoch = 20, lr=5e-7):
    ...
    for epoch in range(0, max_epoch):
        ...
        for i, (image, text, idx) in enumerate(train_loader):
            ...
            match_loss = matching_loss(image_features, hypo_feature, img_head, txt_head, idx, temp, i2tLoss, t2iLoss)
        ...
        with torch.no_grad():
            ...
            for i in range(0, num_text, text_bs):
                ...
                adapt_txt = txt_head(text_embed)
                text_embed = 0.2 * adapt_txt + 0.8 * text_embed
            ...
            for image, img_id in test_loader:
                ...
                adapt_img = img_head(image_embed)
                image_embed = 0.2 * adapt_img + 0.8 * image_embed
...
def matching_loss(image_features, hypo_feature, img_head, txt_head, idx, temp, i2tLoss, t2iLoss):
    bs = image_features.size(0)
    adapt_img = img_head(image_features)
    adapt_txt = txt_head(hypo_feature)
    image_features = 0.2 * adapt_img + 0.8 * image_features
    hypo_feature = 0.2 * adapt_txt + 0.8 * hypo_feature
...
def main():
    ...
    # CLIP
    clip_model, preprocess = clip.load(cfg['backbone'], "cuda:6")
    clip_model.eval()
    embed_dim = 512
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=10, num_workers=8, shuffle=True)
