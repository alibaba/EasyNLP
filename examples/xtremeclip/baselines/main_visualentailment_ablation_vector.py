...
import clip
from utils_visualentailment_ablation_vector import *
import base64
...
def run_tip_adapter_F(cfg, cache_keys, cache_values, test_features, test_labels, clip_model, train_loader_F):
    ...
    optimizer = torch.optim.AdamW(
        [
            {"params": adapter.parameters(), "lr": 6e-3}
        ],
        lr=cfg['lr'],
        betas=(0.9, 0.999),
        eps=1e-4,
    ...
    for train_idx in range(cfg['train_epoch']):
        ...
        for i, (images, hypothesis, premise, target) in enumerate(tqdm(train_loader_F)):
            ...
            adapter_weights = torch.cat([image_features, hypo_feature], dim=-1)#fusion_feat(hypo_feature, image_features)
