...
import clip
from utils_visualentailment_tipadapter import *
import base64
...
def run_tip_adapter_F(cfg, cache_keys, cache_values, test_features, test_labels, clip_model, train_loader_F):
    ...
    adapter = nn.Linear(cache_keys.size(0), cache_keys.size(1), bias=False).to(clip_model.dtype)
    ...
    optimizer = torch.optim.AdamW(
        [
            {"params": clip_model.parameters(), 'lr': 0},
