...
import clip
from utils_eurosat_adaption_vector import *
...
def run_tip_adapter_F(cfg, clss_head, test_features, test_labels, clip_weights,
    ...
    for train_idx in range(max_epoch):
        ...
        for i, (images, target) in enumerate(tqdm(train_loader_F)):
            ...
            final_feat = torch.cat([img_feat, txt_feat], dim=-1)#fusion_feat(txt_feat, img_feat)
        ...
        test_final_feat = torch.cat([test_img_feat, test_txt_feat], dim=-1)#fusion_feat(test_txt_feat, test_img_feat)
