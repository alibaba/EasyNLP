...
def get_arguments():
    ...
    parser.add_argument('--config', default='/home/moming/code/VITCLIP/configs/eurosat.yaml', dest='config', help='settings of Tip-Adapter in yaml format')
...
def run_tip_adapter_F(cfg, clss_head, test_features, test_labels, clip_weights,
    ...
    adapter = nn.Linear(clip_weights.shape[0], clip_weights.shape[1], bias=False).to(clip_model.dtype).cuda(device_id)
    adapter.weight = nn.Parameter(clip_weights.t())
    layers = []
    optimizer_grouped_parameters = [{"params": adapter.parameters(), "lr": 2e-3}]
    # optimizer_grouped_parameters = []
    named_params = list(clip_model.named_parameters())
    for i in range(12):
        layers.append(list(filter(lambda item: "resblocks.%s." % str(i) in item[0], named_params)))
    lr = 5e-7
    no_decay = ["bias", "ln"]
    for layer in layers[::-1]:
        ...
        optimizer_grouped_parameters += [
            {
                "params": [p for n, p in layer if not any(nd in n for nd in no_decay)],
                "weight_decay": 0.01,
                "lr": lr,
            },
            {
                "params": [p for n, p in layer if any(nd in n for nd in no_decay)],
                "weight_decay": 0.0,
                "lr": lr,
            },
        ]
    ...
    for train_idx in range(20):
        ...
        for i, (images, target) in enumerate(tqdm(train_loader_F)):
            ...
            # img_feat = image_features.unsqueeze(1).repeat(1, 10, 1)
            ...
            # log_prob = torch.softmax(logits, dim=-1)#torch.log_softmax(logits, dim=-1)
        ...
        # test_img_feat = test_features.unsqueeze(1).repeat(1, 10, 1)
...
def main():
    ...
    # CLIP
    clip_model, preprocess = clip.load(cfg['backbone'], 'cuda:4')
    clip_model.eval()
