...
EVAL_SAVE_DIR = '/home/moming/data/flickr/adapter/'
...
def run_tip_adapter(clip_model, clss_head, train_loader, test_loader, max_epoch = 20, lr=5e-7):
    ...
    for epoch in range(0, max_epoch):
        ...
        for i, (image, text, idx) in enumerate(train_loader):
            ...
            #### contrastive loss
            contra_loss = contrastive_loss(image_features, hypo_feature, idx, temp, i2tLoss, t2iLoss)
        ...
        with torch.no_grad():
            ...
            for i in range(0, num_text, text_bs):
                ...
                # adapt_txt_feat = adapter_txt_up(text_embed)
                # adapt_txt_feat = relu(adapt_txt_feat)
                # adapt_txt_feat = adapter_txt_down(adapt_txt_feat)
                # text_embed = 0.2 * adapt_txt_feat + 0.8 * text_embed
            ...
            for image, img_id in test_loader:
                ...
                adapt_image_features = adapter_up(image_embed)
                adapt_image_features = relu(adapt_image_features)
                adapt_image_features = adapter_down(adapt_image_features)
                adapt_image_features = relu(adapt_image_features)
                image_embed = 0.2 * adapt_image_features + 0.8 * image_embed
...
def contrastive_loss(image_features, hypo_feature, idx, temp, i2tLoss, t2iLoss):
...
def main():
    ...
    # clip_model.eval()
    embed_dim = 512
    itm_head = nn.Linear(embed_dim*5, 2)
    # nn.Sequential(
    #     nn.Linear(embed_dim * 5, embed_dim * 25),
    #     nn.LayerNorm(embed_dim * 25),
    #     nn.GELU(),
    #     nn.Linear(embed_dim * 25, 2)
    # )
    # for m in itm_head.modules():
