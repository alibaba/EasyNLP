...
EVAL_SAVE_DIR = '/home/moming/data/flickr/ours/'
...
def run_tip_adapter(clip_model, clss_head, train_loader, test_loader, max_epoch = 200, lr=5e-7):
    ...
    for epoch in range(0, max_epoch):
        ...
        for i, (image, text, idx) in enumerate(train_loader):
            ...
            if i % 200 == 0 and i != 0:
                print("contrastive loss:" + str(contra_loss.item()))
...
def matching_loss(image_features, hypo_feature, idx, clss_head, temp, device_id):
    ...
    for b in range(bs):
        ...
        neg_fusion_feat.append(fusion_feat(neg_feat, img_feat.repeat(bs-1, 1)))
...
def contrastive_loss(image_features, hypo_feature, idx, temp):
    ...
    contra_logits = image_features @ hypo_feature.t() * temp.exp()
    contra_i2t_logits = torch.softmax(contra_logits, dim=-1)
    ...
    contra_t2i_logits = torch.softmax(contra_logits.t(), dim=-1)
...
def main():
    ...
    clip_model.eval()
    embed_dim = 512
    itm_head = nn.Linear(embed_dim*5, 2, bias=False)
    ...
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=64, num_workers=16, shuffle=False)
