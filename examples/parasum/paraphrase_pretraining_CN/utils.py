...
def get_data_path(mode, encoder, data_path):
    paths = {}
    if mode == 'train':
        paths['train'] = os.path.join(data_path,'lcqmc_train_' + encoder + '.jsonl')
        paths['val']   = os.path.join(data_path,'lcqmc_val_' + encoder + '.jsonl')
    else:
        paths['test']  = os.path.join(data_path,'lcqmc_val_' + encoder + '.jsonl')
    return paths
