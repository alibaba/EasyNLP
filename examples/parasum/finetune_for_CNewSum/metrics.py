...
_ROUGE_PATH = '../rouge/pythonrouge/RELEASE-1.5.5/'
...
class MarginRankingLoss(LossBase):      
    ...
    def get_loss(self, score, summary_score):
        ...
        ones = torch.ones(score.size()).cuda(score.device) if score.device.type != 'cpu' else torch.ones(score.size()).to(score.device)
        loss_func = torch.nn.MarginRankingLoss(0.0)
        clss_func = torch.nn.BCELoss(reduction='mean')
        clss_label=torch.zeros(score.size()).to(score.device)
        clss_label[:,0]=1
        clss_loss = clss_func(score, clss_label)
        ...
        for i in range(1, n):
            pos_score = score[:, :-i]
            neg_score = score[:, i:]
            #cov_pos = summary_covrage_score[:, :-i].contiguous().view(-1)
            #cov_neg = summary_covrage_score[:, i:].contiguous().view(-1)
            #cov_core = torch.sub(cov_pos, cov_neg)
            ...
            #TotalLoss += loss_func(pos_score + cov_pos*0.3, neg_score + cov_neg*0.3, ones)
            # TotalLoss += loss_func(pos_score, neg_score, cov_core)
        ...
        pos_score = pos_score.contiguous().view(-1)
        neg_score = neg_score.contiguous().view(-1)
        #ref_cov = summary_covrage_score.contiguous().view(-1)
        ones = torch.ones(pos_score.size()).cuda(score.device) if score.device.type != 'cpu' else torch.ones(pos_score.size()).to(score.device)
        #gold_cov = torch.ones(pos_score.size()).to(pos_score.dtype).to(pos_score.device)
        loss_func = torch.nn.MarginRankingLoss(0.0)
        TotalLoss += loss_func(pos_score, neg_score, ones)
        TotalLoss += clss_loss
...
class ValidMetric(MetricBase):
    ...
    def fast_rouge(self, dec, ref):
        if dec == '' or ref == '':
            return 0.0
        dec = " ".join(dec.split()[:700])
        ref = " ".join(ref.split()[:700])
