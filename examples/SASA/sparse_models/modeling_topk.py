...
from easynlp.modelzoo.file_utils import (
    ModelOutput,
...
from easynlp.modelzoo.modeling_outputs import (
...
from easynlp.modelzoo.modeling_utils import PreTrainedModel, apply_chunking_to_forward
from easynlp.modelzoo.utils import logging
from .configuration_topk import TopkConfig
...
TOPK_PRETRAINED_MODEL_ARCHIVE_LIST = [
...
class TopkEmbeddings(nn.Module):
    ...
    def forward(
        ...
        # when its auto-generated, registered buffer helps users when tracing the models without passing token_type_ids, solves
        # issue #5664
...
class TopkSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in TopkModel forward() function)
...
class TopkBlockSparseAttention(nn.Module):
    ...
    def forward(
        ...
        context_layer, attention_probs = self.Topk_block_sparse_attention(
            query_layer,
            key_layer,
            value_layer,
            topk_attn,
    ...
    def Topk_block_sparse_attention(
        self,
        query_layer,
        key_layer,
        value_layer,
        ...
        # Topk block-sparse attention as suggested in paper
        ...
        #     rand_attn = self._Topk_block_rand_mask_with_head(
        ...
        # [bsz, n_heads, from_block_size, -1] x [bsz, n_heads, to_seq_len, -1] ==> [bsz, n_heads, from_block_sze, to_seq_len]
    ...
    def _Topk_block_rand_mask_with_head(
        self,
        from_seq_length,
        to_seq_length,
        from_block_size,
...
# Copied from transformers.models.bert.modeling_bert.BertSelfOutput with Bert->Topk
...
class TopkAttention(nn.Module):
    ...
    def forward(
        ...
        if self.attention_type == "original_full":
            ...
            ), "Topk cannot be used as a decoder when config.attention_type != 'original_full'"
            self_outputs = self.self(
                hidden_states, topk_attn, band_mask, from_mask, to_mask, from_blocked_mask, to_blocked_mask, output_attentions
            )
...
# Copied from transformers.models.bert.modeling_bert.BertIntermediate with Bert->Topk
...
# Copied from transformers.models.bert.modeling_bert.BertOutput with Bert->Topk
...
class TopkEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [TopkLayer(config, seed=layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
# Copied from transformers.models.bert.modeling_bert.BertPredictionHeadTransform with Bert->Topk
...
# Copied from transformers.models.bert.modeling_bert.BertLMPredictionHead with Bert->Topk
...
# Copied from transformers.models.bert.modeling_bert.BertOnlyMLMHead with Bert->Topk
...
# Copied from transformers.models.bert.modeling_bert.BertOnlyNSPHead with Bert->Topk
...
# Copied from transformers.models.bert.modeling_bert.BertPreTrainingHeads with Bert->Topk
...
class TopkPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = TopkConfig
    load_tf_weights = load_tf_weights_in_topk
...
@dataclass
class TopkForPreTrainingOutput(ModelOutput):
    """
    Output type of :class:`~transformers.TopkForPreTraining`.
...
@dataclass
class TopkForQuestionAnsweringModelOutput(ModelOutput):
    """
...
@add_start_docstrings(
    "The bare Topk Model transformer outputting raw hidden-states without any specific head on top.",
    BIG_BIRD_START_DOCSTRING,
)
class TopkModel(TopkPreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = TopkEmbeddings(config)
        ...
        if self.attention_type != "original_full" and config.add_cross_attention:
            logger.warning(
                "When using `TopkForCausalLM` as decoder, then `attention_type` must be `original_full`. Setting `attention_type=original_full`"
            )
    ...
    def _pad_to_block_size(
        ...
        """A helper function to pad tokens and mask to work with implementation of Topk block-sparse attention."""
        # padding
        ...
        if padding_len > 0:
            ...
            if position_ids is not None:
                # pad with position_id = pad_token_id as in modeling_Topk.TopkEmbeddings
...
class TopkForPreTraining(TopkPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.bert = TopkModel(config, add_pooling_layer=True)
        self.cls = TopkPreTrainingHeads(config)
    ...
    @add_start_docstrings_to_model_forward(BIG_BIRD_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @replace_return_docstrings(output_type=TopkForPreTrainingOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids=None,
        attention_mask=None,
        ...
        return TopkForPreTrainingOutput(
            loss=total_loss,
...
@add_start_docstrings("""Topk Model with a `language modeling` head on top. """, BIG_BIRD_START_DOCSTRING)
class TopkForMaskedLM(TopkPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        if config.is_decoder:
            logger.warning(
                "If you want to use `TopkForMaskedLM` make sure `config.is_decoder=False` for "
                "bi-directional self-attention."
            )

        self.bert = TopkModel(config)
        self.cls = TopkOnlyMLMHead(config)
...
@add_start_docstrings(
    """Topk Model with a `language modeling` head on top for CLM fine-tuning. """, BIG_BIRD_START_DOCSTRING
)
class TopkForCausalLM(TopkPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning("If you want to use `TopkForCausalLM` as a standalone, add `is_decoder=True.`")
...
@add_start_docstrings(
    """
    Topk Model transformer with a sequence classification/regression head on top (a linear layer on top of the
    pooled output) e.g. for GLUE tasks.
    """,
    BIG_BIRD_START_DOCSTRING,
)
class TopkForSequenceClassification(TopkPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
        self.bert = TopkModel(config)
        self.classifier = TopkClassificationHead(config)
...
@add_start_docstrings(
    """
    Topk Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a
    softmax) e.g. for RocStories/SWAG tasks.
    """,
    BIG_BIRD_START_DOCSTRING,
)
class TopkForMultipleChoice(TopkPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
@add_start_docstrings(
    """
    Topk Model with a token classification head on top (a linear layer on top of the hidden-states output) e.g. for
    Named-Entity-Recognition (NER) tasks.
    """,
    BIG_BIRD_START_DOCSTRING,
)
class TopkForTokenClassification(TopkPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
...
@add_start_docstrings(
    """
    Topk Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
    layers on top of the hidden-states output to compute `span start logits` and `span end logits`).
    """,
    BIG_BIRD_START_DOCSTRING,
)
class TopkForQuestionAnswering(TopkPreTrainedModel):
    def __init__(self, config, add_pooling_layer=False):
        ...
        self.bert = TopkModel(config, add_pooling_layer=add_pooling_layer)
        self.qa_classifier = TopkForQuestionAnsweringHead(config)

        self.init_weights()

    @add_start_docstrings_to_model_forward(BIG_BIRD_INPUTS_DOCSTRING.format("(batch_size, sequence_length)"))
    @add_code_sample_docstrings(
        checkpoint="google/Topk-base-trivia-itc",
        output_type=TopkForQuestionAnsweringModelOutput,
        config_class=_CONFIG_FOR_DOC,
    )
    def forward(
        self,
        ...
        return TopkForQuestionAnsweringModelOutput(
            loss=total_loss,
